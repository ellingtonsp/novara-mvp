...
/**
 * Novara MVP - Deployment Health Monitor
 * Monitors all deployment environments using unified configuration
 */
...
const CONFIG = {
  ...
  logFile: path.join(PROJECT_PATHS.logs, 'deployment-monitor.log'),
  statusFile: path.join(PROJECT_PATHS.logs, 'deployment-status.json')
};
...
function makeRequest(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const requestOptions = {
      ...
      headers: {
        'User-Agent': 'Novara-Deployment-Monitor/2.0',
        ...options.headers
      },
...
async function checkEnvironment(envName, envConfig) {
  ...
  for (const endpoint of CONFIG.healthEndpoints) {
    try {
      const result = await checkEndpoint(envConfig.backend.url, endpoint);
      results.backend.endpoints[endpoint] = result;
      backendChecks.push(result.success);
    } catch (error) {
      log(`Error checking ${envName} backend ${endpoint}: ${error.message}`, 'ERROR');
      results.backend.endpoints[endpoint] = {
        success: false,
        error: error.message
      };
      backendChecks.push(false);
    }
  }
  ...
  // Check frontend
  try {
    const frontendResponse = await makeRequest(envConfig.frontend.url);
    ...
    results.frontend.statusCode = frontendResponse.statusCode;
  } catch (error) {
    log(`Error checking ${envName} frontend: ${error.message}`, 'ERROR');
...
if (require.main === module) {
  ...
  if (args.includes('--continuous') || args.includes('-c')) {
    ...
    console.log('Novara Deployment Monitor v2.0');
    console.log('');
    console.log('Usage:');
